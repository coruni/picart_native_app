# ArticleControllerGetDislikedArticles200ResponseDataDataInnerCategoryParent


## Properties

Name | Type | Description | Notes
------------ | ------------- | ------------- | -------------
**id** | **number** |  | [default to undefined]
**name** | **string** |  | [default to undefined]
**description** | [**Null**](Null.md) |  | [default to undefined]
**parentId** | [**Null**](Null.md) |  | [default to undefined]
**link** | [**Null**](Null.md) |  | [default to undefined]
**avatar** | **string** |  | [default to undefined]
**background** | **string** |  | [default to undefined]
**cover** | **string** |  | [default to undefined]
**sort** | **number** |  | [default to undefined]
**status** | **string** |  | [default to undefined]
**articleCount** | **number** |  | [default to undefined]
**followCount** | **number** |  | [default to undefined]
**createdAt** | **string** |  | [default to undefined]
**updatedAt** | **string** |  | [default to undefined]

## Example

```typescript
import { ArticleControllerGetDislikedArticles200ResponseDataDataInnerCategoryParent } from './api';

const instance: ArticleControllerGetDislikedArticles200ResponseDataDataInnerCategoryParent = {
    id,
    name,
    description,
    parentId,
    link,
    avatar,
    background,
    cover,
    sort,
    status,
    articleCount,
    followCount,
    createdAt,
    updatedAt,
};
```

[[Back to Model list]](../README.md#documentation-for-models) [[Back to API list]](../README.md#documentation-for-api-endpoints) [[Back to README]](../README.md)
